...
def generate_pbtech_gpu_url():
    ...
    page = requests.get(
        "https://www.pbtech.co.nz/category/components/video-cards/shop-all?o=lowest_price&pg=1#sort_group_form")
    ...
    for i in range(1, max+1):
        pbtech_gpu_url.append(
            f"https://www.pbtech.co.nz/category/components/video-cards/shop-all?o=lowest_price&pg={i}#sort_group_form")
    return pbtech_gpu_url


def generate_mightyape_gpu_url():
    print("Finding pages for Mightyape...")
    page = requests.get("https://www.mightyape.co.nz/computers/graphics-cards/all")
    soup = BeautifulSoup(page.content, "html.parser")
    page_nums_scrape = soup.find(class_="pagination").find_all("li")
    max = None
    mightyape_gpu_url = []
    for page_num in page_nums_scrape:
        if page_num.text.isdigit():
            ...
            if max == None or num > max:
                max = num
    for i in range(1, max+1):
        mightyape_gpu_url.append(f"https://www.mightyape.co.nz/computers/graphics-cards/all?page={i}")
    return mightyape_gpu_url
...
if __name__ == "__main__":
    # This part of code is only for testing. Run from main.py instead
    print(generate_mightyape_gpu_url())
